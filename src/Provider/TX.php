...
//         $sig = Cache::tag('TX-IM')->get($prefix.$this->userName);
        $sig = Cache::get($prefix.$this->userName);
        if(!$sig){//缓存中没有签名
            $config = $this->config;
            $api = new TLSSigAPIv2($this->config['appid'],$this->config['secret']);;
            $sig = $api->genSig($this->userName);//生成usersig
//             Cache::tag('TX-IM')->set($prefix.$this->userName,$sig,15551000);
            Cache::set($prefix.$this->userName,$sig,15551000);
        }
        return $sig;
    }
